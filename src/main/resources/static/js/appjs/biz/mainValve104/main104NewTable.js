...
var prefix3 = "/biz/mainValve104"
$(function() {
    load3();
});

function load3() {
    $('#mainValve104NewTable')
        .bootstrapTable(
            {
...
function reLoad3() {
    $('#mainValve104NewTable').bootstrapTable('refresh');
}
function add3() {
    // layer.open({
    //     type : 2,
...
function batchRemove3() {
    var rows = $('#mainValve104NewTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
    if (rows.length == 0) {
        layer.msg("请选择要删除的数据");
        return;
    }
