...
function reLoad() {
	$('#taskInfoTaskListTable').bootstrapTable('refresh');
}
function add() {
	// iframe层
	var page = layer.open({
...
function batchRemove() {
	var rows = $('#taskInfoTaskListTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
